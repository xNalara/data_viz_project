...
\newthought{As for the other visualizations,} we only have the use of each color on every painting. To represent the evolution of the color usage through the seasons, we have to sum over all the paintings of a season to have for each season 18 attributes with a color as a key and a number between 0 and 13 as value.
...
\newthought{As our database does not evolve,} we could have precomputed the values and save some computing time, but with this approach it could be applied to evolving databases.
